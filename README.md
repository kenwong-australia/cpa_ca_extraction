...
CA ANZ loads the first batch from the site’s **`GetMembers`** call, then paginates by clicking **Load more** (replaying `token`+`offset` yourself often returns 403/500). **Use `--headed`** (headless often hits Cloudflare / reCAPTCHA). **Default jitter for `ca_anz` is 10–25 seconds** between **Load more** clicks (slower than `cpa_au`’s 5–15); override with **`--jitter-min-seconds`** / **`--jitter-max-seconds`**.
...
Between rows, the scraper waits a **uniform random pause** after returning to the list and before opening the next practice. **Defaults** are **5–15 seconds** (implementation plan §3.1). Override with **`--jitter-min-seconds`** and **`--jitter-max-seconds`** (same bounds apply between Phase 3 seed rows and between listing rows).
...
- `--jitter-min-seconds S` / **`--jitter-max-seconds S`** — min/max **uniform random delay** (seconds) between Phase 3 seeds and between practice rows within a search (defaults: `5` and `15`). Use **wider** ranges (e.g. `15`–`35`) if you hit Cloudflare rate limits; `min` must be ≤ `max`.
...
- **Slow down:** pass **`--jitter-min-seconds`** / **`--jitter-max-seconds`** with higher values than the defaults (e.g. `15` and `35`, or `20` and `45`) so there are fewer requests per minute.
...
- **Between locations:** same **random delay** as between practices (§3.1): defaults **5–15 s**, overridable with **`--jitter-min-seconds`** / **`--jitter-max-seconds`**.
...
#   --jitter-min-seconds 15 --jitter-max-seconds 35
```
